...
int InitClient(const char *adresseIP, int port) {
    ...
    if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        ...
        // Ici on simplifie, on assume que l'erreur est gérée plus haut ou
        // ignorée pour l'instant perror("Erreur Connect"); return -1;
    }
...
int AttendreClient(int socketServeur) {
    ...
    int clientSock =
        accept(socketServeur, (struct sockaddr *)&clientAddr, &len);
