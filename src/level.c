...
void init_lab(Block blocks[NUM_BLOCKS][NUM_BLOCKS]) {
  ...
  for (int i = 0; i < NUM_BLOCKS; i++) {
    for (int j = 0; j < NUM_BLOCKS; j++) {
      blocks[i][j].pos = (Vector3){
          i * 3 - offset, 1.0f,
          j * 3 -
              offset};  // Centre du cube = i*2 et j*2 pour doubler la taille
...
void creer_lab(Block blocks[NUM_BLOCKS][NUM_BLOCKS]) {
  ...
  while (!pilevide()) {
    int voisins[4][2] = {{i + 2, j}, {i - 2, j}, {i, j + 2}, {i, j - 2}};
    ...
    for (int j = 0; j < NUM_BLOCKS; j++) {
      ...
      if (blocks[i][j].isWall) {
        ...
        DrawModelEx(floorModel, (Vector3){pos.x, 0.0f, pos.z},
                    (Vector3){0, 1, 0}, 0.0f, (Vector3){3.0f, 0.001f, 3.0f},
                    WHITE);
      }
    }
  }
  ...
  for (int i = NUM_BLOCKS / 3; i < NUM_BLOCKS * 2 / 3; i++) {
    for (int j = NUM_BLOCKS / 3; j < NUM_BLOCKS * 2 / 3; j++) {
      ...
      if (rand() % 100 < 10) {
        ...
        blocks[i][j].pos.y = 1.0f;
        blocks[i][j].color = BLACK;  // On s'assure qu'il a la bonne couleur
      } else {
        // Dans 95% des cas, on vide l'espace pour faire la grande salle
        blocks[i][j].isWall = false;
        blocks[i][j].pos.y = -20.0f;  // On cache le bloc sous le sol
      }
    }
  }
}