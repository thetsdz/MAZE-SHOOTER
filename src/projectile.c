    ...
    for (int i = 0; i < MAX_PROJ; i++) {
        ...
        if (projs[i].type == PROJ_GRENADE) {
            ...
            if (projs[i].pos.y != 0)
                projs[i].pos =
                    Vector3Add(projs[i].pos,
                               distance); // on met a jours pos attention si
                                          // pos.y=0, elle y reste
            else {
            ...
            if (projs[i].pos.y < projs[i].radius &&
                ...
                projs[i].pos.y =
                    projs[i].radius; // avoir une belle balle en surface
                if (projs[i].vel.y < -0.5f) {
                    projs[i].vel.y =
                        projs[i].vel.y * -elasticite; // el famoso rebondo
                ...
                if (-0.2f < projs[i].vel.y &&
                    projs[i].vel.y <
                        0.2) { // velocité y trop faible alors immobile
                    projs[i].vel.y = 0;
                }
            }
            projs[i].life -= dt;
            ...
            if ((*autre)->type == ENTITY_REMOTE_PLAYER) {
                ...
                for (int j = 0; j < 18; j++) {
                    ...
                    if (fabsf(projs[i].pos.x - (*autre)[j].pos.x) < (r + h) &&
                        fabsf(projs[i].pos.y - ((*autre)[j].pos.y + h)) <
                            (r + h) &&
                        ...
                        if ((*autre)[j].health <= 0) {
                            *score += 1;
                            ...
                            (*autre)[j].pos =
                                (Vector3){(float)(rand() % NUM_BLOCKS), 10.0f,
        ...
        // 2. LES BALLES ENNEMIES (Bot ou Remote) me touchent MOI
                    ...
                    if (projs[i].owner == OWNER_BOT && player->health <= 0) {
                        ...
                        if (player->life <= 0) {
                            ...
                            *currentScreen = GAME_OVER;
                            ...
                            player->pos = (Vector3){
                                1.5f, 10.0f, 1.5f}; // Position de respawn solo
            ...
            // B. Vérification de la collision avec d'autres Bots (seulement si
            // le projectile appartient à un bot et n'a pas déjà touché le
            // joueur)
            if (!aToucheJoueur && projs[i].owner == OWNER_BOT) {
                for (int j = 0; j < 18; j++) {
                    if ((*autre)[j].type != ENTITY_BOT)
                        continue;
                    ...
                    if (fabsf(projs[i].pos.x - (*autre)[j].pos.x) < (r + h) &&
                        ...
                        break; // Le projectile a touché ce bot, on arrête de
                               // tester les autres
                    }
                }
            }
            ...
            if (aToucheJoueur)
                continue; // Passe au projectile suivant si le joueur a été
                          // touché
        }
        // pour la grenade on desactive uniquement si sa vie est finie pas quand
        // le bot meurt sinon pas de zone d'explosion
        if (projs[i].life <= 0) {
            projs[i].active = false;
        }
