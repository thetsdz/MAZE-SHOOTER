...
void GererGameOver(GameScreen *currentScreen, int score) {
    ...
    snprintf(scoretext, sizeof(scoretext),
             "Score de fin de partie : %d points.", score);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == 2) {
            texteADessiner = scoretext; // Pour la 3ème case, on utilise ton
                                        // texte formaté avec le score
        } else {
            texteADessiner =
                texteBoutonsDefaite[i]; // Pour les autres, on utilise
...
void GererVictoire(GameScreen *currentScreen, int score) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == 2) {
            ...
            texteADessiner =
                texteBoutonsVictoire[i]; // Pour les autres, on utilise
                                         // le tableau "GAME OVER", etc.
        }
