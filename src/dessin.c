    ...
    for (int i = 0; i < NUM_BLOCKS; i++) {
        for (int j = 0; j < NUM_BLOCKS; j++) {
            ...
            if (dotH < 1)
                dotH = 1;
            int dotX =
                minimapX + (int)((b.pos.x - originX) * scaleX) - dotW / 2;
            int dotY =
                minimapY + (int)((b.pos.z - originZ) * scaleY) - dotH / 2;
...
static void DrawHUD(Entity player) {
  ...
  int py = 10;
    ...
    /* Barre munitions */
    float ammoRatio =
        (float)player.ammo / (float)player.armeEquipee.munitionsMax;
...
void UpdateDessinGame(Entity bot[18], Block blocks[NUM_BLOCKS][NUM_BLOCKS],
    ...
    for (int b = 0; b < 18; b++) {
        ...
        transform =
            MatrixMultiply(transform, MatrixRotateY(angleFinal * DEG2RAD));
