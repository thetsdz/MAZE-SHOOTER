...
void minimap(Entity player, Entity bot[18], Block blocks[NUM_BLOCKS][NUM_BLOCKS]) {
    ...
    DrawText("CARTE", minimapX + MINIMAP_W/2 - labelW/2,
             minimapY - 14, 10, COL_LABEL);
    ...
    for (int i = 0; i < NUM_BLOCKS; i++) {
        for (int j = 0; j < NUM_BLOCKS; j++) {
            ...
            int dotW = (int)(b.width * scaleX); if (dotW < 1) dotW = 1;
            int dotH = (int)(b.depth * scaleY); if (dotH < 1) dotH = 1;
...
static void DrawHUD(Entity player, int score) {
    ...
    Color hpColor = (hpRatio > 0.5f) ? COL_HP_HI : (hpRatio > 0.25f) ? COL_HP_MID : COL_HP_LO;
    DrawBar(PX + PAD, py + 22, PW - PAD*2, 10, hpRatio, hpColor);
    DrawText(TextFormat("%d / %d", player.health, player.maxHealth),
             PX + PAD, py + 38, 10, hpColor);
    ...
    const char* ammoBadge = TextFormat("%d / %d", player.ammo,
    ...
    if (score >= SCORE_TRADE && player.armeEquipee.munitionsMax < MAX_PROJ)
        DrawText("[E] +2 mun  -100 pts", PX + PW/2, py + 48, 10, COL_HINT_GOLD);
    ...
    else if (score < SCORE_TRADE)
        DrawText(TextFormat("Amelioration a %d pts (actuel: %d)",
                            SCORE_TRADE, score), PX, py, 10, COL_HINT);
}
...
void UpdateDessinGame(Entity bot[18], Block blocks[NUM_BLOCKS][NUM_BLOCKS],
    ...
    transform = MatrixMultiply(transform, MatrixRotateX(bot[b].pitch * DEG2RAD));
