...
void UpdatePlayer(Entity *player, Block blocks[NUM_BLOCKS][NUM_BLOCKS],
    ...
    // Calcul du vecteur "Devant" (Forward) uniquement sur le plan horizontal
    // (XZ) Utile pour se déplacer sans s'envoler quand on regarde en l'air
    ...
    // vecteur est 1.41 (racine de 2). On doit le ramener à 1.0 pour ne pas
    // courir plus vite en diagonale.
    ...
    if ((*ennemi)->type == ENTITY_REMOTE_PLAYER) {
        ...
        if (collideX && collideY && collideZ) {
            ...
            if (player->velocityY < 0 &&
                player->pos.y - playerHalf >= enemyTop) {
            ...
            // CAS 3 : Collision latérale (Glissement fluide)
                ...
                float overlapX = (enemyHalf + playerHalf) -
                                 fabsf(nextPos.x - (*ennemi)->pos.x);
                float overlapZ = (enemyHalf + playerHalf) -
                                 fabsf(nextPos.z - (*ennemi)->pos.z);
        ...
        for (int i = 0; i < 18; i++) {
            ...
            if (collideX && collideY && collideZ) {
                ...
                // CAS 3 : Collision latérale (Glissement fluide)
                    ...
                    float overlapX = (enemyHalf + playerHalf) -
                                     fabsf(nextPos.x - (*ennemi)[i].pos.x);
                    float overlapZ = (enemyHalf + playerHalf) -
                                     fabsf(nextPos.z - (*ennemi)[i].pos.z);

                    if (overlapX < overlapZ) {
                        if (nextPos.x < (*ennemi)[i].pos.x)
                            nextPos.x =
                                (*ennemi)[i].pos.x - enemyHalf - playerHalf;
                        else
                            nextPos.x =
                                (*ennemi)[i].pos.x + enemyHalf + playerHalf;
                    } else {
                        if (nextPos.z < (*ennemi)[i].pos.z)
                            nextPos.z =
                                (*ennemi)[i].pos.z - enemyHalf - playerHalf;
                        else
                            nextPos.z =
                                (*ennemi)[i].pos.z + enemyHalf + playerHalf;
                    }
    ...
    for (int i = 0; i < NUM_BLOCKS; i++) {
        for (int j = 0; j < NUM_BLOCKS; j++) {
            ...
            // Vérification : Est-ce que le cube "joueur" chevauche le cube
            // "bloc" sur TOUS les axes ?
            ...
            if (collideX && collideY && collideZ) {
                ...
                // CAS 2 : On cogne le bloc par le bas (Saut)
                    ...
                    player->velocityY =
                        0; // On s'arrête net (on se cogne la tête)
                }
                // CAS 3 : Collision latérale (Mur)
                else {
                    // Calcule du taux de penetration dans le bloc sur chaque
                    // axe fabsf() donne la valeur absolue
                    float overlapX =
                        (halfX + playerHalf) - fabsf(nextPos.x - b.pos.x);
                    float overlapZ =
                        (halfZ + playerHalf) - fabsf(nextPos.z - b.pos.z);
                    ...
                    if (overlapX < overlapZ) {
                        ...
                        // Si on est à gauche du mur, on se met à sa gauche,
                        // sinon à sa droite
                        ...
                        // IMPORTANT : On ne touche PAS à nextPos.z ! (si vous y
                        // touchez je vous bute)
                    } else {
    ...
    for (int i = 0; i < NUM_BLOCKS; i++) {
        for (int j = 0; j < NUM_BLOCKS; j++) {
            ...
            if (playerBottom >= top && nextPos.y - playerHalf <= top) {
                if (top > closestGround)
                    closestGround = top;
            }
        }
    }

    // Appliquer la gravité et la position sur le sol le plus haut sous le
    // joueur
