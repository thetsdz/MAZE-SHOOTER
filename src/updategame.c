...
void ChangementArme(Entity* joueur) {
  if (joueur->score>=10) joueur->armeUnlock[1]=0;
  if (joueur->score>=5) joueur->armeUnlock[0]=0;
  if (joueur->score>=20) joueur->armeUnlock[2]=0;
  // F1 : Pistolet
  if (IsKeyPressed(KEY_F1 && joueur->armeEquipee.type!=PISTOLET)) {
  ...
  // F2 : Fusil
  if (IsKeyPressed(KEY_F3) &&  joueur->armeUnlock[1]==0 && joueur->armeEquipee.type!=FUSIL) {
    joueur->armeEquipee = ObtenirModeleArme(FUSIL);
    joueur->ammo = joueur->armeEquipee.munitionsMax;
    joueur->chronoTir = 0;
  }
  // F3 : Sniper
  if (IsKeyPressed(KEY_F2) && joueur->armeUnlock[0]==0 && joueur->armeEquipee.type!=SNIPER ) {
...
void UpdateGame(Entity *player, Entity bot[18],Heal heal[10],
  ...
  UpdatePlayer(player, blocks, camera, &bot_ptr);
    ...
    for(int i = 0; i < 10; i++) {
        ...
        if (armeUnlock!=0) {
          switch (armeUnlock){
            case 1 : player->armeUnlock[1]=0; break; // par convention le tableau represente sniper,fusil,grenade mais fusil vaut 1 (bref mal organisé je sais, cf heal.c)
