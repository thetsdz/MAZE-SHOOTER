    ...
    */
    ...
    for (int i = 0; i < NUM_BLOCKS; i++) {
        for (int j = 0; j < NUM_BLOCKS; j++) {
            Block b = blocks[i][j];
            if (!b.isWall)
                continue; // Optimisation pour vérifier que les murs
            ...
            if (collideX && collideY && collideZ) {
                ...
                if (boss->velocityY < 0 && boss->pos.y - bossHalf >= top) {
                    ...
                    boss->onGround = true;
                           ...
                           boss->pos.y + bossHalf <= bosstom) {
                    nextPos.y = bosstom - bossHalf;
                    boss->velocityY = 0;
                } else {
                    float overlapX =
                        (halfX + bossHalf) - fabsf(nextPos.x - b.pos.x);
                    float overlapZ =
                        (halfZ + bossHalf) - fabsf(nextPos.z - b.pos.z);
    ...
    float bossBottom = nextPos.y - bossHalf;
    ...
    for (int i = 0; i < NUM_BLOCKS; i++) {
        for (int j = 0; j < NUM_BLOCKS; j++) {
            Block b = blocks[i][j];
            if (b.isWall)
                continue; // On vérifie le sol sur les espaces vides, Ignore les
                          // murs pleins, cherche les sols
            float top = 0.1f; // Le sol par défaut dessiné dans votre level.c
                              // fait Y = ~0, top = ~0.1
            if (bossBottom >= top && nextPos.y - bossHalf <= top) {
    ...
    if (bossBottom < closestGround && boss->velocityY <= 0) {
        nextPos.y = closestGround; // bossHalf;
        boss->velocityY = 0;
        boss->onGround = true;
    }
