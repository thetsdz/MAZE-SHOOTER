...
void DessinerLobbyMultijoueur(ReseauState* netState) {
  ...
  if (netState->socket == -1 && !rechercheEnCours && !saisieIPEnCours) {
    ...
    DrawText(infoText, sw / 2 - MeasureText(infoText, 20) / 2, sh / 2 + 50, 20,
             GRAY);
    const char* warnText =
        "Attention: Le jeu va figer quelques secondes "
...
void partie_multijoueur(Entity* player, Entity* remotePlayer, Heal heal[10],
  ...
  if (!netState->connected) {
    ...
    UpdateMultijoueur(player, remotePlayer, heal, blocks, projs, camera,
                      netState, currentScreen);

    if (*currentScreen == GAME_OVER || *currentScreen == VICTOIRE) {
      *jeuInitialise = false;
    }
...
void DessinerMultijoueur(Entity* player, Entity* remotePlayer, Heal heal[10],
                         ...
                         Model floorModel, Model botModel, Model tabModels[4],Texture2D iconesArmes[]) {
  if (!netState->connected) {
