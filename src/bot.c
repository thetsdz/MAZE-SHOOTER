...
void UpdateBot(Entity* bot, Block blocks[NUM_BLOCKS][NUM_BLOCKS],
  ...
  */
  ...
  // --- AJOUT D'UN TIMER POUR SOULAGER LE PROCESSEUR ---
  static float timerBFS = 0.0f;
  timerBFS += dt;

  // 2. --- LOGIQUE DE NAVIGATION (PATHFINDING) ---
  if (!playerVisible && timerBFS > 0.5f) { // Le bot ne calcule son chemin que toutes les 0.5 secondes
  ...
  */
  ...
  float oldBotBottom = bot->pos.y - botHalf;  // Ancienne position Y
  float newBotBottom = nextPos.y - botHalf;   // Nouvelle position Y
  ...
  for (int i = 0; i < NUM_BLOCKS; i++) {
    for (int j = 0; j < NUM_BLOCKS; j++) {
      ...
      if (oldBotBottom >= top && newBotBottom <= top) {
