...
void UpdateBot(Entity *bot, Block blocks[NUM_BLOCKS][NUM_BLOCKS],
    ...
    */
    ...
    // Déplacement basique : Le bot avance doucement vers le joueur (zombie
    ...
    if (playerVisible && distToPlayer <= 5.0f) {
        ...
        // Tolérance (0.5) pour éviter que le bot tremble une fois arrivé au
        ...
        for (int j = 0; j < NUM_BLOCKS; j++) {
            ...
            if (collideX && collideY && collideZ) {
                ...
                if (bot->velocityY < 0 && bot->pos.y - botHalf >= top) {
                    ...
                    bot->velocityY = 0;
                    bot->onGround = true;
                } else if (bot->velocityY > 0 &&
                           bot->pos.y + botHalf <= bottom) {
                    nextPos.y = bottom - botHalf;
                    bot->velocityY = 0;
                } else {
                    float overlapX =
                        (halfX + botHalf) - fabsf(nextPos.x - b.pos.x);
                    float overlapZ =
                        (halfZ + botHalf) - fabsf(nextPos.z - b.pos.z);
